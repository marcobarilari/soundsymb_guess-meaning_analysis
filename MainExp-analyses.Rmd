---
title: "Semantic Processing in SC and EB - Main Analysis"
author: "Minah Chang"
date: "3 March, 2022"
...
```
"The average raw accuracy rate was 56.3% for the blind and 56.2% for the sighted control group before inversion of the scores for native accuracy. After the adjustment, the average accuracy rate increased to 62.4% for the blind and 62.6% for the sighted control group."
...
"We found no significant difference between the performance of two groups in guessing the meaning of words (β = -0.04, SE = 0.15, z = -0.24, p = .812) - main effect of group is not significant. We also found no significant difference between the performance of two groups across the symbolic categories (β =0.01, SE = 0.01, z =0.70, p = .487)."

According to your other paper, we could use:

(z = -.0.24, p = .812, odds ratio = 0.96, LL = 0.71, UL = 1.30) &
(z = 0.70, p = .487, odds ratio = 1.01, LL = 0.98, UL = 1.04)

However, according to my stats book we are supposed to report B(SE), LL, odds ratio, and UL.

Note: P values don't have a leading 0
Chi square APA style:
X2 (degress of freedom, N = sample size) = chi-square statistic value, p = p value.

A chi-square test of independence showed that there was no significant difference between the blind and sighted groups in their ability to guess the meaning of foreign words (X2 [1, N = 2] = 0.01, p = .920) across all semantic categories (X2 [1, N = 2] = 0.48, p = .487).
...
ggplot(data = adjAccuracy_tidy, 
       aes(x = as.factor(meaning), y = nativeAccuracy_mean, fill = group)) +
  geom_bar(stat="identity", position=position_dodge()) +
  theme_classic() +
  scale_fill_manual(values=c('grey52','darkorange1'), labels=c('Blind', 'Sighted')) +
  scale_y_continuous(limits=c(0, 0.85), breaks=seq(0, 0.85, 0.1), expand = c(0,0)) +
